...
class Tracker:
    ...
    def __init__(self, metric, metric2, max_iou_distance=0.9, max_age=30, n_init=3, _lambda=0, ema_alpha=0.9, mc_lambda=0.995):
    ...
    def update(self, detections, classes, confidences, recFeature=None):
        ...
        selection = self.selectTarget(detections, recFeature)
        ...
        for item in selection:
            ...
            break # To make sure only 1 object is selected
        
        if len(selection)>0:
            print("------------------------")
            print(f"Matched Track={self.tracks[selection[0][0]].track_id}")
            # print("------------------------")
        ...
        self.metric2.partial_fit(np.asarray(features), np.asarray(targets), active_targets)
    ...
    def selectTarget(self, detections, recFeature=None, ACCURACY=0.1):

        def gated_metric(tracks, dets, track_indices, detection_indices, ):

            mod_arr  = dets[-1].feature.copy()
            for i,item in enumerate(mod_arr):    mod_arr[i] = recFeature[i]
            a = np.zeros((512,), dtype=float)
            features = np.array([mod_arr for i in detection_indices])
            targets  = np.array([tracks[i].track_id for i in track_indices])

            cost_matrix = self.metric2.distance2(features, targets)
            cost_matrix = linear_assignment.gate_cost_matrix(cost_matrix, tracks, dets, track_indices, detection_indices)
            
            # row = np.where(cost_matrix2==np.min(cost_matrix2))[0][0]
            # col = np.where(cost_matrix2==np.min(cost_matrix2))[1][0]
            # minVal = cost_matrix2[row][col]
        ...
        #         gated_metric, self.metric2.matching_threshold, self.max_age,
        #         self.tracks, detections, confirmed_tracks)
        matches_a, _ , _ = \
            linear_assignment.matching_cascade(
                gated_metric, ACCURACY, self.max_age,
                self.tracks, detections, confirmed_tracks)
    ...
    def _initiate_track(self, detection, class_id, conf):
        ...
        print("(topLeft_x, topLeft_y, width, height)=" , detection.tlwh) 
