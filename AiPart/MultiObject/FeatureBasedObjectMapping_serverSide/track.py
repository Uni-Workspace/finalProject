...
def run(
    ...
    receivedFeature = np.array([float(i) for i in data.strip("[]").split(",")])
    # =================================================================================================
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                t4 = time_sync()
                outputs[i] = strongsort_list[i].update(xywhs.cpu(), confs.cpu(), clss.cpu(), im0, recFeature=receivedFeature)
                t5 = time_sync()
                dt[3] += t5 - t4
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if len(strongsort_list[0].tracker.selection)>0: # if any object has been selected
                            selectedID = strongsort_list[0].tracker.tracks[strongsort_list[0].tracker.selection[0][0]].track_id # fetch the selected object id
                        else:
                            selectedID = None
